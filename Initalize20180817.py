...
# 执行sql语句
	try:
		with connection.cursor() as cursor:
			# 执行sql语句，进行查询
			sql = """select ci.name,replace(GROUP_CONCAT(ce2.value),',',';') MCCMNC,replace(GROUP_CONCAT(ce3.value),',',';') ChineseName,replace(GROUP_CONCAT(ce1.value),',',';') TADIG,replace(GROUP_CONCAT(ni.value),',',';') DRAIMSIPrefix from subscribedservice ss left join customerinfo ci on ci.id=ss.customerid left join customerextra ce1 on ce1.customerid=ci.id and ce1.attribute='PMN' left join customerextra ce2 on ce2.customerid=ci.id and ce2.attribute='MCCMNC' left join customerextra ce3 on ce3.customerid=ci.id and ce3.attribute='ChineseName' left join neinfo ni on ni.pkgid=ss.id and ni.item='DRAIMSIPrefix' where ss.service='Diameter_Signalling' group by ci.name;"""
			cursor.execute(sql)
			# 获取查询结果
			results = cursor.fetchall()
			#没有设置默认自动提交，需要主动提交，以保存所执行的语句
		...
		print("Can't connect to CCB DB to obtian extra info")
		results=""
		return(results)

	import csv

	with open('.\\file\ccb_extrainfo.csv', 'w',newline='') as csvfile:
		spamwriter = csv.writer(csvfile)
		string=[]
		for keys in results[1]:
			string.append(keys)
		...
		for row in results:
			...
			for t in temp_list:
				if t not in new_list:
					new_list.append(t)
			s=";"
			string.append(s.join(new_list))
			
			spamwriter.writerow(string)
	results=csv2dict('.\\file\ccb_extrainfo.csv')
	return(results)
...
#append OSSDB with LIST name and write to DB.csv
	...
	#insert TADIG
	OSSDB_LIST3=[]
	EXTRA=CCB_EXTRA_INFO()
	for entry in OSSDB_LIST2:
		for extra in EXTRA:
			if entry['name']==extra['name']:
				entry['tagid']=extra['TADIG']
		OSSDB_LIST3.append(entry)
	...
	with open(".\\file\DB.csv", 'w',newline='') as csvfile:
		spamwriter = csv.writer(csvfile)
		string=[]
		for key in OSSDB_LIST3[0].keys():
			string.append(key)
		spamwriter.writerow(string)
		for row in OSSDB_LIST3:
			string=[]
...
# 执行sql语句
	try:
		with connection.cursor() as cursor:
			# 执行sql语句，进行查询
			sql = """select a.o_name,a.o_ssid,a.r_name,a.r_ssid,a.status,a.electing_date,a.o_real_hub,a.dra1,a.dra2,a.dra3,a.r_real_hub,a.update_time from lte_route as a  where a.status =('"""+status+"""')"""
			cursor.execute(sql)
			# 获取查询结果
			results = cursor.fetchall()
			#for row in results:
