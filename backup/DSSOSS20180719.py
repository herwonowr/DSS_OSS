...
class OPEN_ROUTE(QMainWindow, Ui_Ui_OPEN_ROUTE_WIN):
	...
	signal_notice_peer=pyqtSignal(str)
...
#Add K2R FUNCTION_B
	...
	def do_loop(self):
		global peer_notification_flag
		peer_notification_flag=0
		while peer_notification_flag==0:
			print("1")
		
	def send_notification_to_hub(self):
		record={}
		ROUTE_PENDING_PEER_LIST=[]
		...
		for route in filtered_route_owner:
			...
			if route['r_real_hub']!='' and age>= 110:
				record['o_name']=route['o_name']
				record['r_name']=route['r_name']
				record['r_real_hub']=route['r_real_hub']
				record['info']=route['o_name']+'<>'+route['o_real_hub']+'<>'+route['dra1']+'<>'+route['dra2']+'<>'+route['dra3']+'<>'+route['r_real_hub']+'<>'+route['r_name']+'<>---'+route['o_owner']+'-----'+str(route['electing_date'])+'-----'+route['status']+'-----'+str(route['update_time'])
				route_info=route['o_name']+'<>'+route['o_real_hub']+'<>'+route['dra1']+'<>'+route['dra2']+'<>'+route['dra3']+'<>'+route['r_real_hub']+'<>'+route['r_name']+'<>---'+route['o_owner']+'-----'+str(route['electing_date'])+'-----'+route['status']+'-----'+str(route['update_time'])
				ROUTE_PENDING_PEER_LIST_DIC.append(copy.deepcopy(record))
				ROUTE_PENDING_PEER_LIST.append(route_info)

		
		userinfodirectory=r'file\electing_hub.txt'
		output = open(userinfodirectory, 'w')
		for row in ROUTE_PENDING_PEER_LIST:
			output.write(row+"\n")
		output.close()
		
		for row in ROUTE_PENDING_PEER_LIST_DIC:
			self.OP_A.setText(row['o_name'])
			self.OP_B.setText(row['r_name'])
			self.update_AB()
			#MESSAGE_OUTPUT("WIND",row['o_name'])
			global peer_notification_tag
			peer_notification_tag=0
			
			self.signal_notice_peer.emit(row['o_name'])

			print("signal sended")
			
	def show_notice_peer_window(self,route):
			notice_peer_window.show()
			print("send mail window show: "+route)
			self.start_loop()

	def start_loop(self):
		global notice_peer_flag
		notice_peer_flag=1
		while notice_peer_flag==1:
...
class Notice_Peer(QMainWindow, Ui_Notice_Peer):
	...
	def __init__(self, parent=None):    
		...
		#self.PROCEED.clicked.connect(self.close)
		#self.CANCEL.clicked.connect(self.alarms_inputed)
	def notice_peer_settext(self,route):
		self.ELECTING_DATE.setText(route)
...
class Main_TabWidget(QTabWidget):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("DSS OSS 201800716")
        #self.setStyleSheet("background:lightgrey")
