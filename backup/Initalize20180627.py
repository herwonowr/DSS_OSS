	...
	try:
		with connection.cursor() as cursor:
			...
			sql = """SELECT ss.id AS ssid, ss.customerid, ci.`name`, ci.`country`,
               (SELECT LOWER(GROUP_CONCAT(`value`)) FROM neinfo WHERE item = 'DRACustomerRealmName' AND pkgid = ss.id) AS realm_name,
