...
# 执行sql语句
	try:
		with connection.cursor() as cursor:
			# 执行sql语句，进行查询
			sql = """select a.o_name,a.o_ssid,a.r_name,a.r_ssid,a.status value from lte_route as a  where a.status in('Commercial','Ready for Testing') and a.o_name ='Bharti Airtel India'"""
			cursor.execute(sql)
			# 获取查询结果
			results = cursor.fetchall()
			for row in results:
				namelist.append( row['o_name'] )
			#没有设置默认自动提交，需要主动提交，以保存所执行的语句
		connection.commit()
		connection.close()
	except pymysql.err.OperationalError:
	...
	with open('.\\file\rmtdb.csv', 'w',newline='') as csvfile:
		spamwriter = csv.writer(csvfile)
		string=[]
		for keys in results[1]:
			string.append(keys)
		...
		for row in results:
			string=[]
			string.append(row['o_name'])
			string.append(row['o_ssid'])
			string.append(row['r_name'])
			string.append(row['r_ssid'])
			string.append(row['status'])
			spamwriter.writerow(string)
