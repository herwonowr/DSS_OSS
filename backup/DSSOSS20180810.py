...
class MASSIVE_RULE(QMainWindow, Ui_MASSIVE_RULE):
	...
	def exec_current_rule(self):
		...
		ruletype=RULES[index]['ruleType'],\
		description=RULES[index]['description'],\
		orighost=RULES[index]['srcHost'],\
		origrealm=RULES[index]['srcRealm'],\
		desthost=RULES[index]['destHost'],\
		destrealm=RULES[index]['destRealm'],\
		srchost=RULES[index]['adjacentSourceRealmName'],\
		srcrealm=RULES[index]['adjacentSourceRealmName'],\
		priority=RULES[index]['priority'],\
		condition=RULES[index]['condition'],\
		consequence=RULES[index]['consequence'])
		Outputlist_1.append(response)
...
class XML(QMainWindow, Ui_XML):
	def __init__(self, parent=None):    
		...
		for DSC in DSC_List:
			self.ComboBox_DSC.addItem(DSC)
		for i in RegionList:
			self.ComboBox_REGION.addItem(i)
			
		self.DOWN_XML.clicked.connect(self.download_xml)
	...
	def download_xml(self):
		...
		#local = os.getcwd()+r'\file\xml\test.xml'
...
class Main_TabWidget(QTabWidget):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("DSS OSS 201800727")
        #self.setStyleSheet("background:lightgrey")
