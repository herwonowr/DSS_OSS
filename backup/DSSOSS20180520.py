...
## code for OPA only start
	...
	def email(self):
		Tolist='DSS_Route_Provision@syniverse.com'
		Subject='New rule provsioned : '+self.OP_A.text()+'-'+self.OP_B.text()
		email_body=self.LOG.toPlainText()
		sende_plain_mail(Tolist,'',Subject,email_body)
...
class UPD_RMT_ROUTE(QMainWindow, Ui_UPD_RMT_ROUTE):
	...
	def __init__(self, parent=None):    
		...
		results=READ_RMT_ROUTE('Bharti Airtel India')
		item = QTableWidgetItem("1,1")
		self.tableWidget.setItem(1,1,item)
		rowindex=0
		for row in results:
			item=QTableWidgetItem(row['r_name'])
			self.tableWidget.setItem(rowindex,0,item)
			self.btn=QPushButton(str(rowindex+1))
			self.btn_lst.append(self.btn)
			self.tableWidget.setCellWidget(rowindex,2,self.btn_lst[rowindex])
			self.btn_lst[rowindex].clicked.connect(lambda:self.showpos())
			#self.btn_lst[rowindex].clicked.connect(lambda:self.showpos(rowindex))
			#self.btn_lst[rowindex].clicked.connect(lambda:self.showpos(self.btn_lst[rowindex].text()))
			#self.btn_lst[rowindex].clicked.connect(lambda:self.showpos(self.btn_lst.text()))
