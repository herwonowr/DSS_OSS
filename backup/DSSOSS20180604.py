...
def FTRHUB(Name1,Name2,realms1,realms2):
	...
	if realms2=='':
		...
		return()
	reply = QMessageBox.question(self, 'This command will create request filter for A->B,A->B.ftrhub,B->A,B->A.ftrhub on all regional DSC\n  RuleEngine of all region need to be reloaded after provision', 'You sure to quit?',QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
...
#Check DECIDE ROUTE	FUNCTION_A
		self.Check_TO_A_Route.clicked.connect(lambda: CHECK_DECIDE_ROUTE2OP_ALL_REGION(self.realm_B.displayText(),self.realm_A.displayText()))
...
#Check DECIDE ROUTE	FUNCTION_B
		self.Check_TO_B_Route.clicked.connect(lambda: CHECK_DECIDE_ROUTE2OP_ALL_REGION(self.realm_A.displayText(),self.realm_B.displayText()))
