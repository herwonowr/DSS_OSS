...
class OPEN_ROUTE(QMainWindow, Ui_Ui_OPEN_ROUTE_WIN):
	...
	def __init__(self,parent=None):
		...
		self.LIST_ROUTE.clicked.connect(lambda:self.listroute())
		self.ROUTE_LIST.currentIndexChanged.connect(self.fill_OPAB)
		self.HIDE_LIST_ROUTE.clicked.connect(self.hide_listroute)
...
#Add K2R FUNCTION_B
	...
	def listroute(self):
		...
		self.ROUTE_INFO.show()
		results=READ_RMT_ROUTE_BY_STATUS(self.ROUTE_STATUS.currentText())
		for RMT_result in results:
			RMT_result['o_owner']=''
			for entry in DB_sheet:
				if entry['name']==RMT_result['o_name']:
					RMT_result['o_owner']=entry['owner']
			FILTERED_ROUTE.append(RMT_result)
		FILTERED_ROUTE_BY_OWNER=[]
		if OWNER.currentText=='ALL':
			for route in FILTERED_ROUTE:
				FILTERED_ROUTE_BY_OWNER.append(route)
				str=route['o_name']+'-----'+route['r_name']+'   '+route['o_owner']
			self.ROUTE_LIST.addItem(str)
