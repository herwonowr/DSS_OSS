...
# 执行sql语句
	...
	with open('hub.csv', 'w',newline='') as csvfile:
		spamwriter = csv.writer(csvfile)
		string=[]
		for keys in results[1]:
			string.append(keys)
...
def INITIALIZE_OSS_DB():
	...
	#OSS2CSV('oss_initial_db.csv')
	OSSDB=csv2dict('oss_initial_db.csv')
	
#append OSSDB with LIST name and write to OSS_LIST.csv
	OSSDB_LIST=[]
	for entry in OSSDB:
		entry['hub']=''
		entry['hub_policy']=''
	...
	for entry in OSSDB_LIST:
		for hubinfo in HUB_TABLE:
			if str(entry['ssid'])==str(hubinfo['ssid']):
				...
				#else:
					#segment=";"
				#if entry['hub_policy']==None:
					#segment2=""
				#else:
					#segment2=';'
				entry['hub']=entry['hub']+segment+hubinfo['hub']
				entry['hub_policy']=entry['hub_policy']+segment2+hubinfo['hubpolicy']
				
	

	import csv

	with open("OSS_LIST.csv", 'w',newline='') as csvfile:
		spamwriter = csv.writer(csvfile)
		string=[]
