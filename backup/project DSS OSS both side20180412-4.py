...
from soap_all_commands_for_dsc import soap_reload_rule_engine,soap_add_decide_route,soap_add_list_cache,soap_reload_listcaches
def GetFileDate(filename):
	import os, time
	year=str(time.localtime(os.path.getctime(filename)).tm_year)
	month=str(time.localtime(os.path.getctime(filename)).tm_mon)
	day=str(time.localtime(os.path.getctime(filename)).tm_mday)
	hour=str(time.localtime(os.path.getctime(filename)).tm_hour)
	minute=str(time.localtime(os.path.getctime(filename)).tm_min)
	second=str(time.localtime(os.path.getctime(filename)).tm_sec)
...
#COMBINED SOAP FUNCTION
def Reload_Region_LIST(region):
	Region2URL_DSC(region)
	Outputlist_1=[]
	Outputlist_2=[]	
	Output1=soap_reload_listcaches("HKG_DSC")
	Output2=soap_reload_listcaches("SNG_DSC")
...
def Reload_Region_RULE(region):
	...
	Outputlist_1=[]
	Outputlist_2=[]
	Output1=SOAP_RELOAD_RULES(SURL1)
	Output2=SOAP_RELOAD_RULES(SURL2)
	Outputlist_1.append(Output1)
	Outputlist_2.append(Output2)
	
	BIOUTPUT("Reload Rules",Outputlist_1,Outputlist_2)	

#Sub Window of open route
...
#DB BUTTON
		...
		#self.DB_DATE.setText(GetFileDate("rule.csv"))
...
## code for OPA only start
		...
		X3 = 200#labe2 start
		X4 = 250#text2 start
		X5 = 350#button3 start

		self.Lable_TADIG_A = QLabel("TADIG_A",self)
		self.Lable_TADIG_A.move (X3,Y_start-Y_step*1)		
		self.TADIG_A = QLineEdit(self)
		self.TADIG_A.setText("")
		self.TADIG_A.setGeometry(QtCore.QRect(X4, Y_start-Y_step*1, QE_length/2, QE_hight))
		...
		self.LIST_A.setText("LIST")
		self.LIST_A.setGeometry(QtCore.QRect(X2,Y_start+Y_step*5, QE_length, QE_hight))
		
		self.Owner_A = QLabel("Owner_A",self)
		self.Owner_A.move(X1,Y_start+Y_step*6)		
		self.Owner_A = QLineEdit(self)
		self.Owner_A.setText("")
		self.Owner_A.setGeometry(QtCore.QRect(X2,Y_start+Y_step*6, QE_length, QE_hight))
...
#COMBO LIST_A WITH DROP DOWN SELECTION
...
#Add LISTCACHE FUNCTION_A
		...
		self.Lable_Region_A = QLabel("Region_A",self)
		...
		self.B_Reams2A_LIST.clicked.connect(lambda: self.Realms2List_Dialog(self.Combo_Region_A.currentText(),self.realm_B.displayText(),self.LIST_A.displayText()))
		
#Check DECIDE ROUTE	FUNCTION_A

		self.Combo_DRA_A = QComboBox(self)
		for i in DRAlist:
			self.Combo_DRA_A.addItem(i)
		
		self.Combo_DRA_A.move(X2,Y_start+Y_step*19)
		self.Combo_DRA_A.setMaxVisibleItems (4)
		self.Lable_DRA_A = QLabel("DECIDE ROUTE",self)
		self.Lable_DRA_A.move(X1,Y_start+Y_step*19)		
...
#Add K2R FUNCTION_A
		...
		self.Lable_Region_K2R_A = QLabel("Add_ListCache",self)
...
## code for OPB only start
		...
		self.Lable_TADIG_B.move (X3,Y_start-Y_step*1)		
		self.TADIG_B = QLineEdit(self)
		self.TADIG_B.setText("")
		self.TADIG_B.setGeometry(QtCore.QRect(X4, Y_start-Y_step*1, QE_length/2, QE_hight))
		...
		self.Owner_B.setGeometry(QtCore.QRect(X2,Y_start+Y_step*6, QE_length, QE_hight))
...
#Add LISTCACHE FUNCTION_B
		...
		self.Lable_Region_B = QLabel("Region_B",self)
		...
		self.A_Reams2B_LIST.clicked.connect(lambda: self.Realms2List_Dialog(self.Combo_Region_B.currentText(),self.realm_A.displayText(),self.LIST_B.displayText()))
		
#Check DECIDE ROUTE	FUNCTION_B

		self.Combo_DRA_B = QComboBox(self)
		for i in DRAlist:
			self.Combo_DRA_B.addItem(i)
		
		self.Combo_DRA_B.move(X2,Y_start+Y_step*19)
		self.Combo_DRA_B.setMaxVisibleItems (4)
		self.Lable_DRA_B = QLabel("DECIDE ROUTE",self)
		self.Lable_DRA_B.move(X1,Y_start+Y_step*19)		

		self.Check_TO_B_Route = QPushButton('Check to A route',self)
...
#Add K2R FUNCTION_B
		...
		self.Lable_Region_K2R_B = QLabel("Add_ListCache",self)
	...
	def update_A(self,ii):
		...
		for row in DB_sheet:
			if row["name"] == OPA_Name:
				...
				self.TECH_COMMENT_A.setText(row["tech_comment"])
				self.TADIG_A.setText(row["TADIG"])
				self.Combo_Region_A.currentIndex=1		
	def update_B(self,ii):
		OPA_Name = self.Combo_Select_B.currentText()
		self.OP_B.setText(OPA_Name)
		...
		for row in DB_sheet:
			if row["name"] == OPA_Name:
				...
				self.TECH_COMMENT_B.setText(row["tech_comment"])
				self.TADIG_B.setText(row["TADIG"])
				self.Combo_Region_B.currentIndex=1

## code for OPB only end
#add realm to list popup window
	def Realms2List_Dialog(self,Region,Realms,LIST_Name):
		Region2URL_DSC(Region)
