...
def ONE_KEY_HUB_ROUTE(Name1,Name2,realms1,realms2,VZW_LISTCACHE_NAME,DESC_HEADER):
	Outputlist_1=[]
	Outputlist_2=[]
	Outputlist_3=[]
	Outputlist_4=[]
...
#Add K2R FUNCTION_B
	...
	def K2R_initial(self):
		reply=QMessageBox.information(self,'Provision K2R Route','Will Provison Request filter for A->B,A->B.ftrhub,B->A,B-A.ftrub on all region\nOn NA DSC,if Verizon,will only insert RP realm into LIST_VERIZON_WIRELESS_K2R_RP_REALM', QMessageBox.Yes | QMessageBox.No)
		if reply==QMessageBox.Yes:
			ONE_KEY_HUB_ROUTE(self.OP_A.text(),self.OP_B.text(),self.realm_A.text(),self.realm_B.text(),'LIST_VERIZON_WIRELESS_K2R_RP_REALM','K2R:%request_filter% ')
		if reply==QMessageBox.No:
			return()
...
class MASSIVE_RULE(QMainWindow, Ui_MASSIVE_RULE):
	...
	def dump_rule(self):
		...
		output_filename=self.ComboBox_DSC.currentText()+'_rule.csv'
		soap_dump_rule_engine(SURL,output_filename)
...
class Main_TabWidget(QTabWidget):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("DSS OSS 201800726")
        #self.setStyleSheet("background:lightgrey")
